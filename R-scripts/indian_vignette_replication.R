...
# Full regression results included in Appendix
...
mturk_neutral2_attention$mode <- "MTurk"
mturk_neutral2_attention$error <- "Control"
mturk_neutral2_attention$incentive <- "No"

mturk_high2_attention$mode <- "MTurk"
mturk_high2_attention$error <- "High"
mturk_high2_attention$incentive <- "No"

high_attention_results_table <- rbind(mturk_neutral2_attention,mturk_high2_attention) %>%
  filter(term != "(Intercept)") %>%
  rename(Coefficient = estimate,
         SE = std.error,
         t = statistic,
...
xtable(high_attention_results_table, 
       digits = 2,
