...
lab.online.sync.ab <- abilityCalc(lab.online.sync[,c("muID","report.rate","ncorrectret","taxrate","treat")])
baseline.uk.ab <- abilityCalc(baseline.uk[,c("muID","report.rate","ncorrectret","taxrate","treat")])
mturk.ds.ab <- abilityCalc(mturk.ds[,c("muID","report.rate","ncorrectret","taxrate","treat")])
cess.online.panel.ab <- abilityCalc(cess.online.panel[,c("muID","report.rate","ncorrectret","taxrate","treat")])

# Models
model.1.lab.online.sync <- glm(report.rate ~ rank + factor(taxrate) +factor(treat), data=lab.online.sync.ab)
model.1.baseline <- glm(report.rate ~ rank + factor(taxrate) +factor(treat), data=baseline.uk.ab)
model.1.mt.ds <- glm(report.rate ~ rank + factor(taxrate) +factor(treat), data=mturk.ds.ab)
model.1.cp.ds <- glm(report.rate ~ rank + factor(taxrate) +factor(treat), data=cess.online.panel.ab)
...
covariates <- c( "Ability Rank" , "20\\% Deduction","30\\% Deduction","No Audit","Constant")
...
clusterTable$coefficient <- c( "Constant","Ability Rank" , "20\% Deduction","30\% Deduction","No Audit")

addtorow <- list()
addtorow$pos <- list(0, 0)
addtorow$command <- c("& \\multicolumn{4}{c}{\\textbf{Wild Cluster Bootstrapped t-Statistics}} & \\multicolumn{4}{c}{\\textbf{Pairs-Clustered Bootstrapped t-Statistics}}\\\\\n",
                      "& Lab & Online Lab & Online UK & MTurk & Lab & Online Lab & Online UK & MTurk \\\\\n")

print(xtable(clusterTable, caption = "Wild and PCB clustered p-values", label = "table:cluster"),
