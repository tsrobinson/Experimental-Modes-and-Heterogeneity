...
################################################ 
### Data Management for CESS Online Sync 
### Feb 2018
### Denise Laroze
################################################
...
#setwd("C:/Users/Denise Laroze P/Dropbox/CESS-Santiago/archive/Modes/Tax Cheating Qualtrics Online Experiment/Data/CESS Panel UK")#################################
setwd("C:/Users/Denise Laroze Prehn/Dropbox/CESS-Santiago/archive/Modes/Tax Cheating Qualtrics Online Experiment/Data/CESS Panel UK")#################################
...
all.round$muID<-paste0(all.round$player, all.round$room)
...
all.round.2$muID<-paste0(all.round.2$player,  all.round.2$room)
...
all.round.3$muID<-paste0(all.round.3$player, all.round.3$room)
...
all.round.4$muID<-paste0(all.round.4$player,  all.round.4$room)
...
r4$muID<-paste0(all.round.4$player, all.round.4$room)
...
i5$muID<-paste0(all.round.5$player, all.round.5$room)
...
all.round.6$muID<-paste0(all.round.6$player, all.round.6$room)
...
df$auditRate<-ifelse(df$room %in% c("room_1512746737533", "room_1512749283503", "room_1512756964102",
                                    "room_1517335306260", "room_1517342473345", "room_1517342805941",
                                    "room_1518120052812", "room_1518174404864" , "room_1518552030222",
                                    "room_1518552145950", "room_1518724804461", "room_1518724853987",
                                    "room_1519761598762", "room_1519761645026", "room_1519761751259",
                                    "room_1519934437432", "room_1519934553101"
                                    ), 0, 0.1) # In these rooms audit rate 0; others audit rate 0.1
df$audited<-df$statusDeclare ## To make it compatible with other modes
df$treat<-ifelse(df$auditRate >0, 1, 2) # treatment= 2 if audit rate 0 and 1 if audit rate above 0
...
df$report.rate.cess.panel<-df$report.rate
df$sample<-"CESS Online Panel - DS"
...
#df<-df[df$correct>0, ] ### Eliminate observations with no correct responses, because they represent a person that did not take part in the module, not hability. 



#write.csv(df, "C:/Users/Denise Laroze P/Dropbox/CESS-Santiago/Archive/Modes/Tax Cheating Qualtrics Online Experiment/Data/CESS_Panel_DS_Dec2017.csv")
write.csv(df, "C:/Users/Denise Laroze Prehn/Dropbox/CESS-Santiago/Archive/Modes/Tax Cheating Qualtrics Online Experiment/Data/CESS_Panel_DS_Feb2018.csv")
